...
const UINT NUM_ELEMENTS = 0x1000000; //0x1000000;
const UINT NUM_ELEMENTS_OUT = 0x800000; //0x1000000;
...
ID3D11Buffer* CreateAndCopyToDebugBuf( ID3D11Device* pDevice, ID3D11DeviceContext* pd3dImmediateContext, ID3D11Buffer* pBuffer, UINT, UINT );
...
BufType g_vBufOut[NUM_ELEMENTS];
...
int __cdecl main()
{
	...
	sprintf(buf, "Cantidad de elementos: %d", NUM_ELEMENTS);
	std::cout << buf << std::endl;
	sprintf(buf, "Tamaño de memoria para buffers de entrada : %d Mb", 2 * sizeof(g_vBuf0) / (1024 * 1024));
	std::cout << buf << std::endl;
	sprintf(buf, "Tamaño de memoria para buffer de salida   : %d Mb", sizeof(g_vBufOut) / (1024 * 1024));
	std::cout << buf << std::endl;
	sprintf(buf, "Tamaño de memoria para buffer intermedio  : %d Mb", NUM_ELEMENTS_OUT * sizeof(BufType) / (1024 * 1024));
	std::cout << buf << std::endl;
	...
	HRESULT crBuf;
	crBuf = CreateStructuredBuffer( g_pDevice, sizeof(BufType), NUM_ELEMENTS, &g_vBuf0[0], &g_pBuf0 );
	crBuf = CreateStructuredBuffer( g_pDevice, sizeof(BufType), NUM_ELEMENTS, &g_vBuf1[0], &g_pBuf1 );
	crBuf = CreateStructuredBuffer( g_pDevice, sizeof(BufType), NUM_ELEMENTS, NULL, &g_pBufResult );
#else
	...
	HRESULT crBufView;
	crBufView = CreateBufferSRV( g_pDevice, g_pBuf0, &g_pBuf0SRV );
	crBufView = CreateBufferSRV( g_pDevice, g_pBuf1, &g_pBuf1SRV );
	crBufView = CreateBufferUAV( g_pDevice, g_pBufResult, &g_pBufResultUAV );
    ...
    // Read back the result from GPU, verify its correctness against result computed by CPU
    {
		...
		UINT i = 0;
		for (UINT start = 0; start < NUM_ELEMENTS; start += NUM_ELEMENTS_OUT)
		{
			i++;
			printf("    Lote %d\n", i);
			UINT cantItems = NUM_ELEMENTS_OUT;
			if (start + cantItems > NUM_ELEMENTS)
			{
				cantItems = NUM_ELEMENTS - start;
			}
			ID3D11Buffer* debugbuf = CreateAndCopyToDebugBuf(g_pDevice, g_pContext, g_pBufResult, start, cantItems);
			//JPE: ID3D11Buffer* debugbuf = g_pBufResult;
			D3D11_MAPPED_SUBRESOURCE MappedResource;
			BufType *p;
			HRESULT hrMap = g_pContext->Map(debugbuf, 0, D3D11_MAP_READ, 0, &MappedResource);

			// Set a break point here and put down the expression "p, 1024" in your watch window to see what has been written out by our CS
			// This is also a common trick to debug CS programs.
			p = (BufType*)MappedResource.pData;

			for (UINT j = 0; j < cantItems; j++)
			{
				g_vBufOut[start + j] = p[j];
			}
        ...
        BOOL bSuccess = TRUE;
        for ( UINT i = 0; i < NUM_ELEMENTS; ++i )
            if ( (g_vBufOut[i].i != g_vBuf0[i].i + g_vBuf1[i].i)
                 || (g_vBufOut[i].f != g_vBuf0[i].f + g_vBuf1[i].f)
#ifdef TEST_DOUBLE
                 || (g_vBufOut[i].d != g_vBuf0[i].d + g_vBuf1[i].d)
#endif
               )
            {
                 printf( "failure\n" );
	...
	printf("Total Time taken: %.3fs\n", (double)(clock() - tStartGlobal) / CLOCKS_PER_SEC);
	...
	for (UINT i = 0; i < NUM_ELEMENTS; ++i)
	{
		g_vBufOut[i].i = g_vBuf0[i].i + g_vBuf1[i].i;
		g_vBufOut[i].f = g_vBuf0[i].f + g_vBuf1[i].f;
		#ifdef TEST_DOUBLE
		g_vBufOut[i].d = g_vBuf0[i].d + g_vBuf1[i].d;
		#endif
...
//-------------------------------------------------------------------------------------- 
ID3D11Buffer* CreateAndCopyToDebugBuf( ID3D11Device* pDevice, ID3D11DeviceContext* pd3dImmediateContext, ID3D11Buffer* pBuffer, UINT startIndex, UINT CantElements )
{
    ...
    desc.BindFlags = 0;
    desc.MiscFlags = 0;
	desc.ByteWidth = (CantElements) * sizeof(BufType);

	D3D11_BOX srcBox;
	srcBox.left = startIndex * sizeof(BufType);
	srcBox.right = (startIndex + CantElements) * sizeof(BufType);
	srcBox.top = 0;
	srcBox.bottom = 1;
	srcBox.back = 1;
	srcBox.front = 0;
    ...
    if ( SUCCEEDED(pDevice->CreateBuffer(&desc, NULL, &debugbuf)) )
    {
		...
		//pd3dImmediateContext->CopySubresourceRegion(debugbuf, 0, 0, 0, 0, pBuffer, 0, NULL);
		pd3dImmediateContext->CopySubresourceRegion(debugbuf, 0, 0, 0, 0, pBuffer, 0, &srcBox);
    }
